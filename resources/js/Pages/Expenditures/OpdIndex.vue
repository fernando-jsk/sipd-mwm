...
import { Search, Eye, ShieldCheck, RotateCcw, Calendar, Coins } from '@lucide/vue';
...
watch([search, searchBy, statusFilter, sortFilter, startDate, endDate], ([newSearch, newSearchBy, newStatus, newSort, newStartDate, newEndDate], oldValue, onCleanup) => {
    const searchTimeout = setTimeout(() => {
        ...
        router.get('/expenditures/opd', params, { preserveState: true, replace: true });
    }, 300);
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="item in expenditures.data" :key="item.id">
                            ...
                            <TableCell>
                                <div v-if="item.opd_number" class="font-semibold text-sm text-amber-600 dark:text-amber-500 font-mono">{{ item.opd_number }}</div>
