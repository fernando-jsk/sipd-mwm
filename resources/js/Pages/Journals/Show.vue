...
const totalDebit = computed(() => {
    return props.journal.details.reduce((sum, row) => sum + parseFloat(row.debit), 0);
});

const totalCredit = computed(() => {
    return props.journal.details.reduce((sum, row) => sum + parseFloat(row.credit), 0);
});
...
const postJournal = () => {
    if (confirm('Apakah Anda yakin ingin mem-posting jurnal ini? Setelah di-posting, jurnal tidak dapat diubah lagi.')) {
        router.post(`/journals/${props.journal.id}/post`, {}, {
            preserveScroll: true
        });
    }
};
...
<template>
    <Head :title="`Detail Jurnal - ${journal.reference_no}`" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between w-full">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="font-semibold text-xl text-secondary dark:text-foreground leading-tight flex items-center gap-2">
                            Detail Jurnal Umum
                            <span v-if="journal.status === 'posted'" class="px-2 py-0.5 text-xs font-medium bg-emerald-100 text-emerald-800 rounded-full dark:bg-emerald-900/30 dark:text-emerald-300 flex items-center w-fit">
                ...
                <div class="flex items-center gap-2">
                    <Button v-if="journal.status === 'draft'" @click="postJournal" variant="default" class="bg-emerald-600 hover:bg-emerald-700">
        ...
        <Card class="max-w-4xl mx-auto">
            <CardHeader class="border-b border-border/50 text-center pb-6 pt-8">
                <h1 class="text-2xl font-bold uppercase tracking-wider text-secondary">BUKTI JURNAL</h1>
                <p class="text-sm text-muted-foreground">{{ formatType(journal.type) }}</p>
            </CardHeader>
                ...
                <div class="grid grid-cols-2 gap-4 mb-8 text-sm">
                    <div>
                        <div class="flex mb-1">
                            ...
                            <span class="font-medium">: {{ journal.reference_no }}</span>
                        </div>
                        <div class="flex mb-1">
                    ...
                    <div>
                        <div class="flex mb-1">
                            ...
                            <span>: {{ journal.created_by?.name || 'Sistem' }}</span>
                        </div>
                        <div class="flex mb-1">
                ...
                <div class="border rounded-md overflow-hidden">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-border/50">
                            <tr v-for="detail in journal.details" :key="detail.id">
