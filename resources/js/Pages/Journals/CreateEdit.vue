...
const isEdit = !!props.journal;

const form = useForm({
    date: props.journal?.date || new Date().toISOString().split('T')[0],
    reference_no: props.journal?.reference_no || props.defaultRef || '',
    description: props.journal?.description || '',
    type: props.journal?.type || 'general',
    details: props.journal?.details?.length ? props.journal.details.map(d => ({
        id: d.id,
    ...
    <AuthenticatedLayout>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <Card>
                ...
                <CardContent class="pt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        <Label for="type">Tipe Jurnal <span class="text-destructive">*</span></Label>
                        <Select v-model="form.type" required>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectItem value="opening_balance">Jurnal Saldo Awal</SelectItem>
                        ...
                        <p class="text-xs text-destructive" v-if="form.errors.type">{{ form.errors.type }}</p>
                    </div>

                    <div class="space-y-2">
                        <Label for="description">Keterangan Umum</Label>
                        <Textarea id="description" v-model="form.description" rows="2" placeholder="Catatan transaksi..." />
            ...
            <!-- Rincian Jurnal -->
            <Card>
                <CardHeader class="pb-3 border-b border-border/50 flex flex-row items-center justify-between space-y-0">
                ...
                <CardContent class="pt-6 p-0 sm:p-6 sm:pt-6">
                ...
                <CardFooter class="bg-muted/20 border-t border-border flex justify-between p-6">
